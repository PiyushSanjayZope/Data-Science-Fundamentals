...
data.desc.head()
data.desc.head().description.apply(lambda x: x.lower())
data.desc.head().description.apply(lambda x: x.upper())
...
data_outliers.dropna(subset=['manufacturer','fuel','transmission', 'title_status','year'], inplace = True)
data_outliers.isnull().sum()/data_outliers.shape[0]

data_outliers.cylinders.fillna(data_outliers.cylinders.median(), inplace = True)
data_outliers.isnull().sum()/data_outliers.shape[0]

data_outliers[['condition','VIN','drive','type']]= data_outliers[['condition','VIN','drive','type']].fillna('n/a')
data_outliers.isnull().sum()/data_outliers.shape[0]

data_outliers.VIN = data_outliers.VIN.apply(lambda x: 'has_vin' if x != 'n/a' else 'no_vin' )

data_final = data_outliers.drop(['url','region','region_url','size','lat','description','long','image_url','model','text_len'], axis = 1)
data_final['constant'] = 1
data_final['age'] = 2021 - data_final.year
...
X2 = data_final[['constant','age','odometer','cylinders']]
...
#actual regression 
X3 = pd.get_dummies(data_final[['constant','age','odometer','cylinders','condition','fuel','VIN','type']])
